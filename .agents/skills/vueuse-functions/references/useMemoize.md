...
import { useMemoize } from "@vueuse/core";
// ---cut---
const getUser = useMemoize(async (userId: number, headers: AxiosRequestHeaders): Promise<UserData> => axios.get(`users/${userId}`, { headers }).then(({ data }) => data), {
 ...
 * @__NO_SIDE_EFFECTS__
 */
export declare function useMemoize<Result, Args extends unknown[]>(resolver: (...args: Args) => Result, options?: UseMemoizeOptions<Result, Args>): UseMemoizeReturn<Result, Args>;
```
